...
function addStep(testid: string, loglevel: string, msg: string, toAllure = true, issueid = undefined) { 
    //toAllure=true means send report to allure also, if toAllure=false it won't send to allure 
    //issueid is used to send issueid like JIRA ID
    let arr = ["info", "debug", "warn", "error"] //this is for logger
    ...
    try {
        ...
        if (loglevel === "error") {
            ...
            //@ts-ignore
            allure.addStep(msg, {}, "failed") // Substep to fail if error   //Here, we're suing allure only if loglevel === "error"
        } else {
